...
## no variance inside cluster => helpful when we have outliers, otherwise don't use it
...
=> recent time = high variance, longer in time = smaller variance => become a single differentiated population

=> we can use the variance to infer time of split
...
=> U-shape ==> bad (but I didn't understand why)
...
=> 2D = way better --> represent spatiality (= isolation by distance)
...
-> K is a parameter to estimate a priory
-> best K = lower cross-entropy
-> very sensitive to ouliers and LD and low-frequency
-> should filer LD within each pop (but we don't know yet) 
-> do multipler runs, pick the best K but take into consideration biology (there could be little diff btw K & K+1 for e.g. but K+1 could make more sense)
